...
class Lexicon():
    ...
    def get_output_text(self):
        # return '\n'.join(sango_sort(list(set(e.as_lexicon_line()
        # for e in self.entries if e.gloss_sg is not None))))
        return '\n'.join(sango_sort(list(set(
            e.as_lexicon_line()
            for e in self.entries.values()
            if e.gloss_sg is not None)))
        )
...
def key_from_value(dict, value):
    # This only works with a 1:1 dictionary; i.e. it only returns the first
    # match.
